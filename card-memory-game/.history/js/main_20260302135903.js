...
// Game Configuration
const BOARD_COLS = 4; // 4 columns (horizontal)
const BOARD_ROWS = 3; // 3 rows (vertical)
const TOTAL_CARDS = BOARD_COLS * BOARD_ROWS; // 12 cards
const TOTAL_PAIRS = TOTAL_CARDS / 2; // 6 pairs
...
function saveBestTime(milliseconds) {
  ...
  if (!currentBest || milliseconds < parseInt(currentBest)) {
    localStorage.setItem("memoryGameBestTime", milliseconds.toString());
    loadBestTime();
  }
}
...
function createShuffledDeck() {
  ...
  // Add one extra card to make 15 total (duplicate of first symbol)
  if (TOTAL_CARDS % 2 !== 0 && symbols.length > 0) {
    deck.push({ id: `${TOTAL_PAIRS}-extra`, symbol: symbols[0], matched: false });
  }
...
function buildBoard() {
  ...
  console.log(`Building board: ${deck.length} cards (${BOARD_COLS}x${BOARD_ROWS})`);
